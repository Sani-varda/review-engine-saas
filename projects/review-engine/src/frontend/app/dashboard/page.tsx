...
import { Loader2, TrendingUp, Star, Users, ArrowUpRight, Share2, LayoutGrid } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Mock checking google connection
  ...
  const handleConnectGoogle = async () => {
    try {
      const res = await fetch(`http://localhost:8000/google/auth-url?business_id=${businessId}`, {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-4">
          <Link 
            ...
            className="bg-white border border-slate-200 text-slate-700 px-6 py-2.5 rounded-xl font-semibold hover:bg-slate-50 transition-all shadow-sm flex items-center gap-2"
          >
            <LayoutGrid size={18} /> Manage Reviews
          </Link>
          <button 
      ...
      {!googleConnected && (
        <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-2xl p-6 text-white flex items-center justify-between shadow-xl shadow-blue-200">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center backdrop-blur-md">
              <Share2 size={24} />
            </div>
            <div>
              <h3 className="text-lg font-bold">Connect Google Business Profile</h3>
              <p className="text-blue-100 text-sm">Integrate with Google to manage and reply to reviews directly from here.</p>
            </div>
          </div>
          <button 
            onClick={handleConnectGoogle}
            className="bg-white text-blue-700 px-6 py-2.5 rounded-xl font-bold hover:bg-blue-50 transition-all active:scale-95"
          >
            Connect Now
          </button>
        </div>
